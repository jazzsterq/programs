...
void solve()
{
    ...
    rep(i,n)
    {
        if(s.count({v[i].first.first,v[i].first.second})||s.count({v[i].first.second,v[i].first.first}))
        {
            continue;
        }
        forf(j,i+1,n)
        {
            ...
            if(v[j].first.first==v[i].first.second&&v[j].first.second==v[i].first.first)
            {
