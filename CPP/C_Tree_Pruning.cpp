...
void solve()
{
    ...
    function <void(ll,ll)> dfs= [&](ll node,ll parent){
        for(auto i: tree[node]){
            if(i!=parent){
                dis[i]={dis[node].first+1,i};
                par[i]=node;
                dfs(i,node);
            }
        }
        if(tree[node].size()==1){
            leaves.pb({dis[node].first,node});
        }
    };
    dis[0]={1,0};
    dfs(0,0);
    ...
    ll ans=INT_MAX;
    ll i=0,ops=0;
    for(auto [dist,leaf]: leaves){
        while(i<n&&dis[i].first<=dist)i++;
        //cout<<i<<" "<<ops<<" "<<endl;
        ans=min(ans,n-i+ops);
        
        while(tree[leaf].size()==1&&leaf!=0)
        {
            ops++;
            tree[par[leaf]].erase(leaf);
            tree[leaf].erase(par[leaf]);
            leaf=par[leaf];
        }
